...
本ツールは「バラバラに届く依頼を、人間の関門を必ず通して在庫・請求へ落とす」ことに徹した
設計です。**AIは補助・確定は人間・判断基準は外出し・起きたときに追える** の4点を、機能だけでなく
データ構造とUI文言の両方で担保しています。

全体像は [README のフロー図／アーキテクチャ図](../README.md) を参照。本書はその補足として、
**図の凡例** と **設計の要点** をまとめます。
...
| 見た目 | 意味 |
|---|---|
| 📥 📦 🧾 などのノード | 処理フローの各段階（届く→読取→確認→確定→在庫→サマリー→月末→請求） |
| 👥 → 🖥️ → 🔐 → 🗄️ | 利用者 → 画面 → サーバー → データ の縦の階層 |
| 実線の矢印 | 主要な流れ（画面 → サーバー → データ） |
| 点線の矢印 | 補助的な流れ（通知・トリガー） |
