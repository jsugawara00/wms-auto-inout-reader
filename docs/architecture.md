...
- 月末スナップショットは確定時点の名称・数量を非正規化コピーで固定（後からマスタが変わっても
  月末表は不変）。
- 請求書は「締め（下書き）→ 確認 → 発行（印刷・送付用の締め）」の3段。ただし中小の実態に合わせ、
  **発行後も「修正のため再開」で締めを解いて再編集→再発行できる**（月末に締めても翌月に荷役料・
  例外請求が動くため。確定＝不変ロックはしない）。再開は理由必須で履歴に残し、確認中の調整も
  原本（計算値）は残したまま表示値だけを重ねます。
