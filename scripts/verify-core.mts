...
{
  const r = await confirmSlip({ slipId: 1, operator: "op01", expectedVersion: 0, allowNegative: false, allowDateMismatch: true });
...
{
  const r = await confirmSlip({ slipId: 2, operator: "op01", expectedVersion: 0, allowNegative: false, allowDateMismatch: true });
  check("確定成功", r.ok === true, r);
...
{
  const r = await confirmSlip({ slipId: 3, operator: "op02", expectedVersion: 0, allowNegative: false, allowDateMismatch: true });
  check("確定成功", r.ok === true, r);
...
{
  const r1 = await confirmSlip({ slipId: 4, operator: "op01", expectedVersion: 0, allowNegative: false, allowDateMismatch: true });
  check("承認なしは negative 警告で停止", !r1.ok && r1.kind === "negative", r1);
  check("警告時は在庫不変 80", (await stockQty("lot_no = :l", { l: "HK-2606" })) === 80);
  const r2 = await confirmSlip({ slipId: 4, operator: "op01", expectedVersion: 0, allowNegative: true, allowDateMismatch: true });
...
console.log("\n— 入出庫日の関門（FB⑥：当日でない伝票は承認制・記録は実際の日） —");
{
  const exPast: Extraction = {
    ...ex2,
    slip_number: "YS-20260711-03",
    movement_date: "2026-07-01", // 書類上の出荷日（過去）
    lines: ex2.lines.map((l) => ({ ...l, quantity: 3 })),
  };
  const r = await intakeExtraction(exPast, "verify-past.pdf", "fax");
  const d1 = await getSlipDetail(r.slipId!);
  check("入出庫日が読取から入る", d1?.slip.movement_date === "2026-07-01", d1?.slip.movement_date);

  // 品目は既存（ex2で登録済み）に自動紐付けされているはず。倉庫のみUI相当で設定
  await db().exec("UPDATE slip_lines SET warehouse_id = 3 WHERE slip_id = :id", { id: r.slipId });
  const d2 = await getSlipDetail(r.slipId!);
  const c1 = await confirmSlip({
    slipId: r.slipId!, operator: "op01", expectedVersion: d2!.slip.version, allowNegative: false,
  });
  check("当日でない入出庫日は date_mismatch で停止", !c1.ok && c1.kind === "date_mismatch", c1);
  const c2 = await confirmSlip({
    slipId: r.slipId!, operator: "op01", expectedVersion: d2!.slip.version, allowNegative: false,
    allowDateMismatch: true,
  });
  check("担当承認で確定成功", c2.ok === true, c2);
  const sum = await getDailySummary("2026-07-01");
  check("サマリーは入出庫日（2026-07-01）に載る", sum.some((s) => s.slip_id === r.slipId), sum.length);
}
...
console.log("\n— サマリー・一覧 —");
{
  // サマリーは入出庫日基準（シード伝票1・2の入出庫日は 2026-07-09）
  const summary = await getDailySummary("2026-07-09");
  check("サマリーが入出庫日基準で出る（7/9の出庫2件）", summary.length >= 2, summary.length);
